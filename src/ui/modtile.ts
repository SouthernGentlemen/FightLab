...
import { tagLine } from "../mods/tags.ts";
...
export function modSquare(definition: ModDefinition, className = "square"): HTMLElement {
  return paintTags(h("span", { class: className, "aria-hidden": "true" }, icon(modIcon(definition.id as ModId), "icon square__glyph")), definition.tags);
}
...
export function modTile(definition: ModDefinition, stars: Stars, owned: number): HTMLButtonElement {
  ...
  const node = h("button", {
    ...
    "aria-label": `${definition.name}, ${tagLine(definition.tags)}, ${RARITY[definition.rarity].label}, ${owned} owned`,
  },
  ...
  h("span", { class: "tile__type" }, tagLine(definition.tags)),
