...
describe("the economy", () => {
  it("buys back at half the price of every copy inside, rounded down, never less than a dollar", () => {
    ...
    expect(sellValue({ mod: "heat-coil", stars: 1 })).toBe(1);
    expect(sellValue({ mod: "heat-coil", stars: 2 })).toBe(4);
    expect(sellValue({ mod: "heat-coil", stars: 3 })).toBe(9);
    expect(sellValue({ mod: "amplifier", stars: 1 })).toBe(4);
  });
  ...
  it("tallies payday line by line: base, result, interest, style and perks", () => {
    ...
    const perks = compileBuild(place(place([], { uid: 1, mod: "crowd-pleaser", stars: 1, rotation: 0, x: 0, y: 0 })!, { uid: 2, mod: "piggy-bank", stars: 1, rotation: 0, x: 0, y: 1 })!);
