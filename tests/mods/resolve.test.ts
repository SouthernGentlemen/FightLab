...
import {
  LEGACY_BATTERY, LEGACY_CINDER, LEGACY_COOLING, LEGACY_DYNAMO, LEGACY_HEAT, LEGACY_RESERVOIR,
  LEGACY_SINK, LEGACY_VENOM, LEGACY_VOID_TAP, LEGACY_WIRE, legacyProgram,
...
describe("resources", () => {
  it("make Heat, more at each star", () => {
    for (const [stars, heat] of [[1, 1], [2, 2], [3, 3]] as const) {
      const prepared = prepareExchange(FRESH, [legacyProgram([LEGACY_HEAT, 0, 0, stars]), NOTHING], ["tech", "tech"]);
  ...
  it("let a consumer spend Heat made in the same exchange, on damage and a Burn that waits for the hit", () => {
    const solar = legacyProgram([LEGACY_HEAT, 0, 0], [LEGACY_CINDER, 0, 1]);
  ...
  it("skip a spend that cannot be paid", () => {
    const prepared = prepareExchange(FRESH, [legacyProgram([LEGACY_CINDER, 0, 0]), NOTHING], ["strike", "strike"]);
    expect(prepared.bonus).toEqual([0, 0]);
    expect(prepared.pending[0]).toEqual([]);
  });

  it("sink Heat into healing, up to the sink's size", () => {
    const prepared = prepareExchange([state({ heat: 5 }), state()], [legacyProgram([LEGACY_SINK, 0, 0]), NOTHING], ["block", "strike"]);
  ...
  it("store Charge up to capacity, and a Battery raises it", () => {
    const dynamo = legacyProgram([LEGACY_DYNAMO, 0, 0, 3]);
    ...
    const stored = legacyProgram([LEGACY_DYNAMO, 0, 0, 3], [LEGACY_BATTERY, 0, 2]);
  ...
  it("spend Charge on damage and Shock", () => {
    const prepared = prepareExchange([state({ charge: 1 }), state()], [legacyProgram([LEGACY_WIRE, 0, 0]), NOTHING], ["strike", "tech"]);
  ...
  it("leech the opponent's Heat or Charge into Void — only what the opponent has, the larger pool first", () => {
    const prepared = prepareExchange(FRESH, [legacyProgram([LEGACY_VOID_TAP, 0, 0, 3]), legacyProgram([LEGACY_HEAT, 0, 0])], ["tech", "tech"]);
    expect(prepared.states[1].heat).toBe(0);
    expect(prepared.states[0].voidCharge).toBe(1);
    const either = prepareExchange([state(), state({ heat: 1, charge: 3 })], [legacyProgram([LEGACY_VOID_TAP, 0, 0, 2]), NOTHING], ["tech", "tech"]);
  ...
  it("leech both ways at once, so the order the fighters are listed in never matters", () => {
    const both = legacyProgram([LEGACY_VOID_TAP, 0, 0], [LEGACY_HEAT, 0, 1]);
...
describe("debuffs", () => {
  ...
  it("have no cap", () => {
    ...
    const venom = legacyProgram([LEGACY_VENOM, 0, 0, 3]);
  ...
  it("land on the opponent only when the move they rode on lands, and cleanse only when the guard holds", () => {
    const prepared = prepareExchange([state({ heat: 1 }), state()], [legacyProgram([LEGACY_CINDER, 0, 0]), NOTHING], ["strike", "block"]);
    expect(settleExchange(prepared, QUIET)[1].burn).toBe(0);
    expect(settleExchange(prepared, PLAYER_LANDS)[1].burn).toBe(2);
    const cooled = prepareExchange([state({ heat: 3, burn: 5 }), state()], [legacyProgram([LEGACY_COOLING, 0, 0]), NOTHING], ["block", "strike"]);
  ...
  it("vent half the Heat at the end of a round, keep Charge and Void, and let Void accrue", () => {
    const reservoir = legacyProgram([LEGACY_RESERVOIR, 0, 0, 2]);
