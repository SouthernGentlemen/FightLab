...
describe("rules text", () => {
  it("folds firing, scale, landing and conditions into new payoff text", () => {
    expect(effectLines(PER_FIXTURES[1], 1)).toEqual([
      "+1 damage per cell on Strike",
    ]);
    expect(effectLines(CONDITION_FIXTURES[0], 1)).toEqual([
      "Burn 2 on a hit if next to Solar",
    ]);
    expect(effectLines(fixture(
      ...
      "Burn 1 per adjacent Solar mod on a hit",
    ]);
  });

  it("writes compact boost and perk rules", () => {
    expect(effectLines(BOOST_FIXTURES[0], 1)).toEqual(["Adjacent mods +1"]);
    expect(effectLines(BOOST_FIXTURES[1], 2)).toEqual([
      "Adjacent Solar mods +2 on Tech",
    ]);
    expect(effectLines(PERK_FIXTURES[0], 1)).toEqual(["+$2 every payday"]);
    expect(effectLines(PERK_FIXTURES[1], 2)).toEqual([
      "+2 free rerolls every day",
    ]);
  });
  ...
  it("names the status from the mod type and describes every landing mode", () => {
    ...
    expect(effectLines(STATUS_FIXTURES[2], 1)).toEqual([
      "Poison 2 if guard holds",
    ]);
    expect(effectLines(STATUS_FIXTURES[3], 1)).toEqual([
      "No status each exchange",
    ]);
    expect(LANDING_FIXTURES.map((definition) => effectLines(definition, 1)[0]))
      .toEqual([
        "Burn 1 on a hit",
        "Burn 1 on a hit",
  ...
  it("fits every registry mod and tasks-029 fixture in three 60-character lines at every star", () => {
    for (const definition of [...DEFINITIONS, ...TASKS_029_FIXTURES]) {
      for (const stars of STARS) {
        const lines = effectLines(definition, stars);
        expect(lines.length, `${definition.id} ★${stars}: ${lines.join(" | ")}`)
          .toBeLessThanOrEqual(3);
        for (const line of lines) {
          expect(line.length, `${definition.id} ★${stars}: ${line}`)
  ...
  it("changes star-scaled new rules at each level", () => {
    const definitions = [
      ...PER_FIXTURES,
      ...BOOST_FIXTURES,
      ...PERK_FIXTURES,
